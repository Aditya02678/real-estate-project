...
useEffect(() => {
  const handleScroll = () => {
    if (window.scrollY > 0) {
      //close the menu if open when user scrolls
...
const{loginWithRedirect ,isAuthenticated,user,logout} =useAuth0()
  ...
  return (
    <header className={`${active ? ' py-1 bg-white shadow-md' : 'py-2 '} mx-auto max-w-[1440px] px-6 lg:px-12  fixed top-0 w-full left-0 right-0 z-50 transition-all duration-200 `}>
      {/* {container} */}
        <div className="flexBetween"> 
          ...
          sm:gap-x-5 bold-16'>
           { !menuOpened ?(
            ...
            { !isAuthenticated ? <button onClick={loginWithRedirect} className ="flexCenter gap-x-2 btn-dark text-white py-2 px-4 mr-5">
